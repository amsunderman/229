...
int main(int argc, char ** argv)
{
    ...
    bool override_tx, override_ty, override_wx, override_wy;

    // integers that store terrain and window dimensions
    int terrain_xl, terrain_xh, terrain_yl, terrain_yh, window_xl, window_xh, window_yl, window_yh;
    ...
    for(int i = 1; i < argc; i++)
    {
        ...
        if(argv[i][0] == '-')
        {
            ...
            else if(current_argument == "-g")
            {
                // used to check type of next argument. should be an integer
                istringstream argument_type_check(argv[++i]);
    ...
    // will output be gol file or visual?
    if(cmd_visual)
        view = new CmdView();
    else
        view = new GOLOut();
