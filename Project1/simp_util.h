...
/* Function used to write to file fp using the simp data from the provided 
 * simp_file structure */
int write_simp_file(FILE * fp, struct simp_file * simp_data);

/* Function used to modify simp_file structure with crop coordinates and 
 * dimensions */
int crop(struct simp_file * simp_data,struct simp_file * cropped, 
	int x, int y, int width, int height);

/* Function used to modify simp_file structure converting it to black and 
 * white */
int bw(struct simp_file * simp_data);

/* Function used to modify simp_file structure by applying a color shift 
 * pattern */
int colorshift(struct simp_file * simp_data, char * pattern);

/* Function used to overlay an image on top of another image at coordinate x, 
 * y */
int overlay(struct simp_file * simp_data, struct simp_file * overlay_data, 
	int x, int y);

/* Support function for overlay used to calculate RGB values with respect to 
 * their alpha values */
void overlay_calculation(unsigned char * color_value1, unsigned char * 
	color_value2, unsigned char * alpha1, unsigned char * alpha2);
